...
def init_db():
    ...
    # Инициализируем все стадии с нулевыми значениями для текущей даты
    ...
    for stage in stages:
        # Используем INSERT OR IGNORE для добавления записей только если их еще нет
        cursor.execute(
            "INSERT OR IGNORE INTO daily_stats (date, stage, count, total_summa) VALUES (?, ?, 0, 0)",
            (today, stage)
        )
    ...
    logger.info(f"База данных инициализирована со всеми стадиями для даты {today}")

# Функция для инициализации стадий для произвольной даты
def init_stages_for_date(date_str):
    conn = sqlite3.connect('webhooks.db')
    cursor = conn.cursor()
    ...
    logger.info(f"Инициализированы все стадии для даты {date_str}")
...
def get_stats():
    ...
    # Инициализируем все стадии для выбранной даты, если их нет
    init_stages_for_date(date)
